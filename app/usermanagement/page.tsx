...
export default function UserManagement() {
  ...
  const fetchCompanies = async () => {
    try {
      const response = await fetch("/api/allcompany"); // Replace with your actual API route
      if (!response.ok) {
  ...
  const handleRead = async (user: User) => {
    if (selectedUser?._id === user._id) {
      setSelectedUser(null); // Toggle off details if already selected
      setCompanyName(null);
      return;
    }
  ...
  const handleCreateUser = async () => {
    try {
      ...
      const data = await response.json();
      alert(data.message);
      fetchUsers(); // Refresh company list
  ...
  const handleUpdate = async (userId: string) => {
    ...
    if (user && user._id) {  // Ensure _id is defined
      setModifiedUser({
        id: user._id || "",
        name: user.name,
        email: user.email || "",
  ...
  const handleUpdateUser = async () => {
    ...
    try {
      ...
      alert(data.message);
      fetchUsers(); 
      setModifiedUser(null); // Close the update form
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-6 relative">
      ...
      <h1 className="text-2xl font-bold mb-6 text-teal-600">User Management</h1>
      ...
      {showForm && (
        <div className="bg-white p-4 shadow-md rounded-lg mb-6">
          ...
          >
            ...
            <div className="mb-4">
              <label htmlFor="Id" className="block text-teal-700">
                Company
              </label>
              <select
                id="Id"
                ...
                value={newUser.Id}
                onChange={handleInputChange}
                className={`w-full p-2 border rounded-lg mt-2 ${newUser.Id ? 'text-blue-700' : ''}`}
                required
              >
                ...
                {companies.map((company) => (
                  <option className="text-blue-700" key={company._id} value={company._id}>
                    {company.name}
                  </option>
                ))}
              </select>
          ...
          <div key={user._id} className="p-4 bg-white shadow-md rounded-lg">
            <div className="flex justify-between items-start">
              ...
              <div className="flex space-x-2">
                ...
                <button
                  onClick={() => {if(!companies.length) fetchCompanies()
            ...
            {modifiedUser && modifiedUser.id === user._id && (
              <div className="bg-white p-4 shadow-md rounded-lg mt-4">
                ...
                >
                  ...
                  <div className="mb-4">
                    <label htmlFor="companyId" className="block text-teal-700">Company</label>
                    <select
                      id="companyId"
                      name="companyId"
                      value={modifiedUser?.companyId || ""}
                      onChange={handleEditInputChange}
                      className={`w-full p-2 border rounded-lg mt-2 ${modifiedUser?.companyId ? 'text-blue-700' : ''}`}
                    >
                      <option value="" disabled>
                        Select a Company
                      </option>
